...
(user@host)-$ singularity exec docker://reslp/abyss:2.2.5 \
               ...
               in="$(pwd)/trimmed/reads.trimmed.pe.1.fastq.gz $(pwd)/trimmed/reads.trimmed.pe.2.fastq.gz" default 2>&1 | tee abyss/abyss.81/abyss.log


(user@host)-$ mkdir abyss/abyss.merged.81
(user@host)-$ singularity exec docker://reslp/abyss:2.2.5 \
               abyss-pe -C abyss/abyss.merged.81 k=81 name=abyss np=4 \
               in="$(pwd)/trimmed/reads.fastp.notmerged.1.fastq.gz $(pwd)/trimmed/reads.fastp.notmerged.2.fastq.gz" \
               se="$(pwd)/trimmed/reads.fastp.merged.fastq.gz $(pwd)/trimmed/reads.trimmed.unpaired.1.fastq.gz $(pwd)/trimmed/reads.trimmed.unpaired.2.fastq.gz" default 2>&1 | tee abyss/abyss.merged.81/abyss.log

(user@host)-$ mkdir abyss/abyss.merged.51
(user@host)-$ singularity exec docker://reslp/abyss:2.2.5 \
               abyss-pe -C abyss/abyss.merged.51 k=51 name=abyss np=4 \
               in="$(pwd)/trimmed/reads.fastp.notmerged.1.fastq.gz $(pwd)/trimmed/reads.fastp.notmerged.2.fastq.gz" \
               se="$(pwd)/trimmed/reads.fastp.merged.fastq.gz $(pwd)/trimmed/reads.trimmed.unpaired.1.fastq.gz $(pwd)/trimmed/reads.trimmed.unpaired.2.fastq.gz" default 2>&1 | tee abyss/abyss.merged.51/abyss.log
...
(user@host)-$ singularity exec docker://chrishah/platanus:v1.2.4 \
               ...
               -f $(pwd)/trimmed/reads.trimmed.pe.1.fastq $(pwd)/trimmed/reads.trimmed.pe.2.fastq -t 4 -m 8 2>&1 | tee platanus/platanus.assemble.log 
(user@host)-$ singularity exec docker://chrishah/platanus:v1.2.4 \
               platanus scaffold -o platanus/platanus -c platanus/platanus_contig.fa -b platanus/platanus_contigBubble.fa \
               -IP1 $(pwd)/trimmed/reads.trimmed.pe.1.fastq $(pwd)/trimmed/reads.trimmed.pe.2.fastq -t 4 2>&1 | tee platanus/platanus.scaffold.log 
(user@host)-$ singularity exec docker://chrishah/platanus:v1.2.4 \
               platanus gap_close -o platanus/platanus -c platanus/platanus_scaffold.fa \
               -IP1 $(pwd)/trimmed/reads.trimmed.pe.1.fastq $(pwd)/trimmed/reads.trimmed.pe.2.fastq -t 4 2>&1 | tee platanus/platanus.gapclose.log 
